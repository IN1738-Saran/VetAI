...
// Gemini API configuration — key lives in .env only, never hardcoded
        ...
        if (mimetype === 'application/pdf') {
            return new Promise((resolve, reject) => {
                ...
                pdfParser.on('pdfParser_dataError', (errData) => {
                    ...
                    resolve(`[Could not extract text from PDF: ${filename}]`);
                });
                ...
                pdfParser.on('pdfParser_dataReady', (pdfData) => {
                    try {
                        ...
                        resolve(text.trim() || `[No text found in PDF: ${filename}]`);
                    } catch (e) {
                        resolve(`[Error processing PDF: ${e.message}]`);
                    }
                });
...
// Extract candidate name and email using Gemini
...
Return ONLY valid JSON, no markdown, no explanations.`;
        ...
        console.log(`✅ Extracted candidate info: ${candidateInfo.candidateName} (${candidateInfo.candidateEmail})`);
        return {
...
try {
    ...
    await fetch(CREATION_WEBHOOK_URL, {
        method: 'POST',
 ...
 * SAVE TRANSCRIPT
 */
app.post('/api/interview/:sessionId/transcript', (req, res) => {
    const { sessionId } = req.params;
...
// Generates a short-lived Gemini Live token for the browser.
// The real GEMINI_API_KEY never leaves this server.
// --------------------------------------------------------------------------------
app.post('/api/interview/:sessionId/token', async (req, res) => {
    ...
    // Step 2 — Generate ephemeral token via direct REST call
    // auth_tokens endpoint requires x-goog-api-key HEADER — ?key= param is rejected
    // when expireTime is valid (server checks expireTime before key, masking the real error)
    try {
        const apiKey = process.env.GEMINI_API_KEY;
        console.log(`🔑 Using API key: ${apiKey ? apiKey.substring(0, 12) + '...' : 'UNDEFINED — check .env'}`);
        ...
        const now = new Date();
        const expireTime = new Date(now.getTime() + 35 * 60 * 1000).toISOString();
        const newSessionExpireTime = new Date(now.getTime() + 2 * 60 * 1000).toISOString();

        console.log(`🕐 expireTime: ${expireTime}`);
        console.log(`🕐 newSessionExpireTime: ${newSessionExpireTime}`);
        ...
        if (!response.ok) {
            ...
            console.error('❌ Token API error:', response.status, errText);
            return res.status(500).json({ message: 'Failed to generate session token. Please try again.' });
        }

        const data = await response.json();
        console.log(`✅ Ephemeral token generated for session ${sessionId}`);
        res.json({ token: data.name });

    } catch (e) {
        console.error('❌ Ephemeral token generation failed:', e.message);
        res.status(500).json({ message: 'Failed to generate session token. Please try again.' });
    }
});
