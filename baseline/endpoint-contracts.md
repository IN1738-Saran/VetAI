...
| Call site | URL | Notes |
|---|---|---|
| `dashboard.html`, `analytics.html` on load | `POST https://n8n.systechusa.com/webhook/dataentry?ts=<timestamp>` | **Sole source of candidate list data.** Response is defensively unwrapped (`data`/`data.data`/`data.result`/`data.candidates`/bare object). **UPDATE 2026-08-13 (Phase 3 live smoke test, 2,903 real records inspected):** confirmed exactly 11 fields exist on every record — the original 9 (`candidatename`, `candidateemail`, `jobtitle`, `overall_score`, `verdict`, `summary`, `status`, `createdat`, `sessionid`) plus two previously-unproven fields, `updatedat` and `reattempt_reason`. `verdict` and `status` are **controlled enums, not free text** — this directly contradicts the assumption baked into `dashboard.html`'s `getVerdictBadgeClass`/`getStatusBadgeClass` (which match against pass/accept/qualified/fail/reject/pending/scheduled/cancelled/failed keywords). Real values: `verdict` ∈ {`strong_fit` (1002), `fit` (900), `borderline` (509, +1 dirty `Borderline`), `weak_fit` (488), `reject` (2, rare), `null` (1, rare)}; `status` ∈ {`Interview Not Scheduled` (1968), `Interview Scheduled` (526), `Interview Completed` (409)} — exactly 3 values, no free text observed. Applying the old keyword heuristic against this real data misclassified ~2,900/2,903 records as "needs review" with a 0% pass rate — `admin-frontend/src/lib/badgeClass.ts` was rewritten against these real values instead of the ported heuristic once this was discovered. No field beyond these 11 was observed — anything else is still unproven and must render a placeholder per plan section 4.3 (sub-scores, strengths/gaps, "top X%", department, seniority, skill tags, question markers, attempts, source — none of these appeared anywhere in the live payload). |
